            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">{{ trans('global.edit') }} Material</h4>
                    </div>
                </div>
            </div>

            <div class="row">
                <form method="POST" action="{{ route("teacher.course-materials.update", $courseMaterial->id) }}" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="card" id="content-card-template">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control js-example-basic-single {{ $errors->has('course_id') ? 'is-invalid' : '' }}" name="course_id" id="course_id" required>
                                        ...
                                        @foreach ($courses as $key => $data)
                                            <option value="{{ $data->id }}"  {{ (old('course_id') == $data->id || $courseMaterial->course_id == $data->id) ? 'selected' : '' }}>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $courseMaterial->title) }}">
                                    @if($errors->has('title'))
                                        <div class="invalid-feedback">
                                            {{ $errors->first('title') }}
                                        </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control js-example-basic-single {{ $errors->has('upload_type') ? 'is-invalid' : '' }}" name="upload_type" id="upload_type" required>
                                        ...
                                        @foreach (App\Models\CourseMaterials::UPLOAD_TYPE as $key => $data)
                                            <option value="{{ $key }}" {{ old('upload_type', $courseMaterial->type) == $key ? 'selected' : '' }}>
                                                {{ $data }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3" id="file_div">
                                    ...
                                    @if ($courseMaterial->type == App\Models\CourseMaterials::TYPE_FILE)
                                        <a href="{{ $courseMaterial->file }}" target="_blank">View Existing File</a>
                                    @endif
                                ...
                                <div class="mb-3" id="url_div">
                                    ...
                                    <input type="text" class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" name="url" id="url" value="{{ old('url', $courseMaterial->url) }}">
                                    @if($errors->has('url'))
                                        <div class="invalid-feedback">
                                            {{ $errors->first('url') }}
                                        </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control {{ $errors->has('remarks') ? 'is-invalid' : '' }}" name="remarks" id="remarks">{{ old('remarks', $courseMaterial->remarks) }}</textarea>
